...
export default function Bestseller({searchKey, brand, priceFilter, viewOption}) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        setLoading(true)
        const data = await getProducts(params) // ✅ sửa tên
        setProducts(data.data)
        setLoading(false)
      } catch (err) {
        console.error(err)
        setError("Có lỗi khi tải dữ liệu bestseller")
        setLoading(false)
      }
    }
  ...
  return (
    <div>
      <h2 className='text-2xl bg-blue-400 text-black mx-4 shadow-xl text-center'>
        BestSeller Products Hà Vũ Đông
      </h2>
